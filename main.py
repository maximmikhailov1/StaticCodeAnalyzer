...
def check_007(s):
    if 'def' in s or 'class' in s:
        return re.match(r'(def)?class?\s*:', s)
    return False


def check_008(s):
    if 'class' in s:
        return re.match(r'class ((^[A-Z][a-z]*)+):', s)
    return False


def check_009(s):
    if 'def' in s:
        return re.match(r'^_{2}[a-z]+(_{2})?$', s) or \
               re.match(r'def ([a-z]*_?)+:', s)
    return False


def check_line(path_name, line, line_number):
    for error_num in error_checkers:
        if error_checkers[error_num](line):
            print(f'{path_name}: Line {line_number}: {error_num} {code[error_num]}')
...
try:
    if os.path.isdir(args.path):
        for path in sorted(os.listdir(args.path)):
            with open(os.path.join(args.path, path), 'r', encoding='utf-8') as file:
                code_line = file.readline()
                i = 1
                while code_line:
                    check_line(os.path.join(args.path, path), code_line, i)
                    code_line = file.readline()
                    i += 1
            empty_lines = 0
    else:
